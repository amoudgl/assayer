"""
MNIST example training script that saves checkpoint every epoch. Checkpoints
saved by the script are fetched by assayer and evaluated using
the `eval_from_checkpoint` method implemented in eval.py.

Run command:
```
python main.py
```
OR like below to specify GPU id
```
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        scheduler.step()
        save_path = os.path.join(args.checkpoint_dir, f"model_epoch{epoch}.pt")
        torch.save(model.state_dict(), save_path)

    if args.save_model:
        save_path = os.path.join(args.checkpoint_dir, "mnist_final.pt")
        torch.save(model.state_dict(), save_path)
